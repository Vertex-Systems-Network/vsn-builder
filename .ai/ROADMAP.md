...
### P1.6 Incremental hotspot decomposition — P1.6g STOREFRONT MUTATION PAYLOAD BOUNDS IMPLEMENTED

**Status:** P1.6a–P1.6f extracted major storefront boundaries and hardened proxy ownership/responses. P1.6g bounds the authenticated public app-proxy mutation body before FormData parsing. `app/storefront/mutationRequest.server.js` enforces a 32 MiB aggregate ceiling using both Content-Length prechecks and streamed byte counting, cancels oversized streams, replays accepted bytes into normal FormData semantics, and returns hardened 413 JSON responses. Experiment metadata is capped at 16 KiB before JSON parsing and wishlist JSON at 2 MiB before parsing. App-proxy authentication still runs before body streaming, and accepted form/wishlist/experiment behavior is otherwise unchanged. The aggregate ceiling intentionally takes precedence over the larger theoretical multi-file combination allowed by per-file settings to bound public memory/CPU exposure. P1.6 remains incremental; future slices should continue one cohesive domain at a time rather than rewriting the proxy.
