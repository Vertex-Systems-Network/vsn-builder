...
### P1.6 Incremental hotspot decomposition — P1.6d STOREFRONT WIDGET-GRID DATA BOUNDARY IMPLEMENTED

**Status:** P1.6a–P1.6c extracted product pagination, dynamic metaobject loading, and content-query behavior. P1.6d now extracts the shared pure Shopify product-node projection into `app/storefront/productMapper.js` and widget-grid request discovery/Admin data loading into `app/storefront/widgetGridData.server.js`. The boundary preserves the existing 40-request cap, normalized grid props, current-collection/search-context short-circuits, recommendation/recent/upsell placeholders, query+limit caches, GraphQL shapes, sort behavior, and per-query failure isolation. Existing collection/all-product query paths now consume the shared mapper rather than a route-local implementation. The proxy retains authentication, loader orchestration, persistence, rendering, and response ownership. Characterization plus security/package guards keep behavior stable and prevent unrelated authority from entering the extracted modules. P1.6 remains incremental; future slices should continue one cohesive domain at a time rather than rewriting the proxy.
