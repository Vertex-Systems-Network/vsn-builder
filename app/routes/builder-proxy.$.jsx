...
function clampProductPageSize(
	...
	return Math.max(
		1,
		Math.min(24, Math.round(numeric)),
	);
}
...
function findCollectionProductSort(
	...
	for (const node of list) {
		if (
			...
			return normalizeProductSort(
				node?.props?.sortBy || fallback,
			);
		}
	...
	return fallback
		? normalizeProductSort(fallback)
		: "";
}

function getCollectionSortConfig(sortBy) {
...
function getAllProductsSortConfig(sortBy) {
	switch (normalizeProductSort(sortBy)) {
		case "oldest":
...
function sortProductsByPrice(products, sortBy) {
	const direction =
		normalizeProductSort(sortBy) === "price-desc"
			? -1
			: 1;
...
function parsePriceSortCursor(cursor) {
	const match = /^price:(\d+)$/.exec(String(cursor || ""));
	return match ? Number(match[1]) : 0;
}

function buildPriceSortPage(products, pageSize, afterCursor) {
