...
async function buildSystemInfo(shop, session, admin, diagnostics, widgetSummary, pageRows = [], setting = null) {
  ...
  const dbStarted = Date.now(); try { await db.$queryRaw`SELECT 1`; database.latencyMs=Date.now()-dbStarted; } catch (error) { database={status:"error",latencyMs:Date.now()-dbStarted,error:error instanceof Error?error.message:"Database query failed"}; }
  ...
  let migrations={count:null,latest:null}; try { const rows=await db.$queryRaw`SELECT migration_name, finished_at FROM "_prisma_migrations" WHERE finished_at IS NOT NULL ORDER BY finished_at DESC`; migrations={count:Array.isArray(rows)?rows.length:null,latest:Array.isArray(rows)&&rows[0]?{name:rows[0].migration_name,finishedAt:rows[0].finished_at}:null}; } catch {}
