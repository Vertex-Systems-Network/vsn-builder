...
function ipv6BigInt(address){
  ...
  if(input.includes(".")){
    ...
    const dotted=input.slice(lastColon+1);const parts=dotted.split(".").map(Number);
...
function publicIpv6(address){
  const value=ipv6BigInt(address);if(value===null||value===0n||value===1n)return false;
  if((value>>121n)===0x7en)return false; // fc00::/7 unique-local
...
function publicIp(address){const family=isIP(String(address||"").replace(/^\[|\]$/g,""));return family===4?publicIpv4(address):family===6?publicIpv6(address):false;}
function blockedHostname(host){const h=String(host||"").replace(/^\[|\]$/g,"").toLowerCase();return h==="localhost"||h.endsWith(".localhost")||h.endsWith(".local")||h.endsWith(".internal");}
...
async function resolvePublicTarget(url){
  ...
  if(isIP(host))rows=[{address:host,family:isIP(host)}];
    ...
    try{rows=await lookup(host,{all:true,verbatim:true});}catch{throw new Error("Could not resolve the inspiration URL safely.");}
  }
...
function pinnedRequest(url,target){
  return new Promise((resolve,reject)=>{
    ...
    const request=transport.request(url,{
      ...
      const status=Number(response.statusCode||0);const headers=response.headers||{};
      ...
      response.on("data",(chunk)=>{
        if(settled)return;total+=chunk.length;
...
async function fetchUrlInspiration(rawUrl){
  ...
  for(let redirects=0;redirects<URL_FETCH_MAX_REDIRECTS;redirects++){
    const target=await resolvePublicTarget(url);const res=await pinnedRequest(url,target);
...
const ELEMENT_SCHEMA={type:"object",additionalProperties:false,required:["ref","parentRef","type","label","text","url","imageUrl","alt","tag","columns","gap","backgroundColor","textColor","fontSize","fontWeight","paddingTop","paddingRight","paddingBottom","paddingLeft","marginTop","marginRight","marginBottom","marginLeft","width","maxWidth","height","borderRadius","direction","align","justify","objectFit"],properties:{
  ref:{type:"string"},parentRef:{type:"string"},type:{type:"string",enum:AI_ALLOWED_TYPES},label:{type:"string"},text:{type:"string"},url:{type:"string",description:"Use only http(s), safe relative/anchor links, or mailto/tel for buttons. Never use javascript:, data:, file:, or blob: URLs."},imageUrl:{type:"string",description:"Use only http(s) or safe relative image URLs. Never use javascript:, data:, file:, or blob: URLs."},alt:{type:"string"},tag:{type:"string"},columns:{type:"number"},gap:{type:"number"},backgroundColor:{type:"string"},textColor:{type:"string"},fontSize:{type:"number"},fontWeight:{type:"number"},paddingTop:{type:"number"},paddingRight:{type:"number"},paddingBottom:{type:"number"},paddingLeft:{type:"number"},marginTop:{type:"number"},marginRight:{type:"number"},marginBottom:{type:"number"},marginLeft:{type:"number"},width:{type:"string"},maxWidth:{type:"string"},height:{type:"string"},borderRadius:{type:"number"},direction:{type:"string"},align:{type:"string"},justify:{type:"string"},objectFit:{type:"string"}
}};
