...
After the command completes, VSN reloads the page and rebuilds the deterministic Quality Report. The response includes authoritative before/after quality summaries, command/revision identifiers, the undo checkpoint revision, and a `resolved` flag based only on the revalidated deterministic finding signature. A successful mutation may therefore still return `applied_unresolved`; execution never claims a fix solely because a command ran.
