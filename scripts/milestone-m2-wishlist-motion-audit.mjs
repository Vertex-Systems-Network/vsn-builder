...
check('App proxy uses signed customer identity',()=>{const route=read('app/routes/builder-proxy.$.jsx');const service=read('app/services/wishlist-proxy.server.js');assert.match(route,/handleWishlistProxyAction/);assert.match(route,/handleWishlistProxyLoader/);assert.match(service,/logged_in_customer_id/);assert.match(service,/wishlist-sync/);assert.match(service,/wishlistResponse/);});
