...
ok(policy.behaviorVersions.page === "page-v1" && policy.behaviorVersions.email === "email-v1" && policy.behaviorVersions.agent === "agent-v1" && policy.behaviorVersions.brand === "brand-extract-v1", "Default behavior versions must be explicit");
...
for (const key of ["VSN_AI_PROVIDER", "VSN_AI_TIMEOUT_MS", "VSN_AI_MAX_RETRIES", "VSN_AI_PAGE_BEHAVIOR_VERSION", "VSN_AI_EMAIL_BEHAVIOR_VERSION", "VSN_AI_AGENT_BEHAVIOR_VERSION", "VSN_AI_BRAND_BEHAVIOR_VERSION"]) ok(envExample.includes(key), `Environment contract missing ${key}`);
