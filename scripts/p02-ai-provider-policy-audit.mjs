...
const reference = resolveAiBehavior({ surface: "reference", operation: "analyze", version: AI_BEHAVIOR_DEFAULTS.reference });
ok(reference.version === "reference-v1", "Reference analysis behavior v1 must remain explicitly selectable");
ok(reference.instructions.includes("untrusted source data") && reference.instructions.includes("rather than copying") && reference.instructions.includes("never publishes"), "Reference behavior must retain prompt-injection, transform-not-copy and no-publish guardrails");
...
ok(policy.behaviorVersions.page === "page-v1" && policy.behaviorVersions.email === "email-v1" && policy.behaviorVersions.agent === "agent-v1" && policy.behaviorVersions.agentContext === "agent-v2" && policy.behaviorVersions.reference === "reference-v1" && policy.behaviorVersions.brand === "brand-extract-v1", "Default behavior versions must be explicit");
...
for (const [label, source] of [["Page AI", pageService], ["Email AI", emailService], ["Editor Agent", agentService], ["Brand extraction", brandExtractionService], ["Reference analysis", referenceService]]) {
  ok(vendorTransportMarkers.every((marker) => !source.includes(marker)), `${label} must not own provider credentials or transport`);
}
ok(pageService.includes("generateStructuredAi") && emailService.includes("generateStructuredAi") && agentService.includes("generateStructuredAi") && brandExtractionService.includes("generateStructuredAi") && referenceService.includes("generateStructuredAi"), "Page, Email, Agent and Brand extraction AI must use the shared provider contract");
ok(
...
for (const key of ["VSN_AI_PROVIDER", "VSN_AI_TIMEOUT_MS", "VSN_AI_MAX_RETRIES", "VSN_AI_PAGE_BEHAVIOR_VERSION", "VSN_AI_EMAIL_BEHAVIOR_VERSION", "VSN_AI_AGENT_BEHAVIOR_VERSION", "VSN_AI_AGENT_CONTEXT_BEHAVIOR_VERSION", "VSN_AI_REFERENCE_BEHAVIOR_VERSION", "VSN_AI_BRAND_BEHAVIOR_VERSION"]) ok(envExample.includes(key), `Environment contract missing ${key}`);
