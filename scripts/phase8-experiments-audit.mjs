...
const proxy=read('app/routes/builder-proxy.$.jsx'); for(const token of ['resolveExperimentRender','data-vsn-experiment-id','data-vsn-experiment-runtime-css','vsnExpPreview']) ok(proxy.includes(token),`Storefront experiment render bridge missing ${token}`);
const secureProxy=read('app/routes/builder-proxy-secure.$.jsx'); for(const token of ['_vsnAction','experiment-event','recordExperimentEvent']) ok(secureProxy.includes(token),`Secure storefront experiment mutation bridge missing ${token}`);
ok(!proxy.includes('_vsnAction')&&!proxy.includes('recordExperimentEvent('),'Legacy storefront loader must not regain experiment mutation authority');
